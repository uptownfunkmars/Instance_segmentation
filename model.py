...
from pth_nms.pth_nms import pth_nms as nms
...
def bbox_overlaps(boxes1, boxes2):
    ...
    y1 = torch.max(b1_y1.cuda(), b2_y1.cuda())[:, 0]
    x1 = torch.max(b1_x1.cuda(), b2_x1.cuda())[:, 0]
    y2 = torch.min(b1_y2.cuda(), b2_y2.cuda())[:, 0]
    x2 = torch.min(b1_x2.cuda(), b2_x2.cuda())[:, 0]
    ...
    # 3. Compute unions
    b1_area = ((b1_y2 - b1_y1) * (b1_x2 - b1_x1)).cuda()
    b2_area = ((b2_y2 - b2_y1) * (b2_x2 - b2_x1)).cuda()
...
def detection_target_layer(proposals, gt_class_ids, gt_boxes, gt_masks, config):
    ...
    # print(roi_iou_max.size())

    # 1. Positive ROIs are those with >= 0.5 IoU with a GT box
    positive_roi_bool = roi_iou_max >= 0.5
    
    # print(torch.nonzero(positive_roi_bool))
    ...
    if torch.nonzero(negative_roi_bool).size()[0]:
        ...
        # 注释掉的为原文部分，有bug若positive_count = 0,则会报错
        ...
        if positive_count > 0:
            ...
            negative_count = int(config.TRAIN_ROIS_PER_IMAGE * (1 - config.ROI_POSITIVE_RATIO))
    ...
    # print("##########roi roi_gt_class_ids deltas masks###########")
    # print(rois, roi_gt_class_ids, deltas, masks)
...
def compute_mrcnn_bbox_loss(target_bbox, target_class_ids, pred_bbox):
    ...
    if target_class_ids.size():
        ...
        target_bbox = target_bbox[indices[:,0].data,:].float()
...
class MaskRCNN(nn.Module):
    ...
    def train_epoch(self, datagenerator, optimizer, steps):
        ...
        for inputs in datagenerator:
            ...
            print("############compute loss result################")
            # print(rpn_class_loss.size(), rpn_bbox_loss.size(), mrcnn_class_loss.size(), mrcnn_bbox_loss.size(), mrcnn_mask_loss.size())           
            ...
            # Progress
            #print("progress")
            #print(loss.data.cpu(), rpn_class_loss.data.cpu(), rpn_bbox_loss.data.cpu(), mrcnn_class_loss.data.cpu(), mrcnn_bbox_loss.data.cpu(), mrcnn_mask_loss.data.cpu())
...
#############################################################################################
            # Progress
            print(loss.data.cpu().size(), rpn_class_loss.data.cpu().size(), rpn_bbox_loss.data.cpu().size(), mrcnn_class_loss.data.cpu().size(), mrcnn_bbox_loss.data.cpu().size(), mrcnn_mask_loss.data.cpu().size())
            printProgressBar(step + 1, steps, prefix="\t{}/{}".format(step + 1, steps),
                             suffix="Complete - loss: {:.5f} - rpn_class_loss: {:.5f} - rpn_bbox_loss: {:.5f} - mrcnn_class_loss: {:.5f} - mrcnn_bbox_loss: {:.5f} - mrcnn_mask_loss: {:.5f}".format(
                                 loss.data.cpu(), rpn_class_loss.data.cpu(), rpn_bbox_loss.data.cpu(),
                                 mrcnn_class_loss.data.cpu(), mrcnn_bbox_loss.data.cpu(),
